...
export default function MessageThread({ chartId }: { chartId: string }) {
  ...
  async function load() {
    const { data } = await supabase
      .from('messages').select('*').eq('chart_id', chartId).order('created_at');
    setMsgs(data ?? []);
  }
  ...
  async function send() {
    ...
    await supabase.from('messages').insert({
      chart_id: chartId, sender_id: profile?.id, sender_role: profile?.role, body,
    });
    load();
  }
        ...
        {msgs.map(m => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start', margin: '6px 0' }}>
              <div style={{
                maxWidth: '74%', padding: '9px 13px', borderRadius: 13, fontSize: 14,
                background: mine ? 'var(--oxblood)' : 'var(--paper-2)',
                color: mine ? '#fff' : 'var(--ink)',
                borderBottomRightRadius: mine ? 4 : 13,
                borderBottomLeftRadius: mine ? 13 : 4,
              }}>
                <div className="tiny" style={{ opacity: .7, marginBottom: 2 }}>
                  {mine ? 'You' : (m.sender_role ?? 'team')}
                </div>
                {m.body}
              </div>
            </div>
